...
def load_fitresults_from_file(filename):
    ...
    for key in tree.GetListOfBranches():
        ...
        if name.startswith("r"):
            ...
            print(indices[name])
            count += 2
            results[name] = [-999, -999, -999, -999, -999]
    ...
    for i, row in enumerate(tree):
        for name in indices:
            ...
            if i == 0:
                ...
                print("*********** results[name][0]", results[name][0])
            elif i == indices[name]:
                results[name][1] = getattr(row, name)
                print("*********** results[name][1]", results[name][1])
            elif i == indices[name]+1:
                results[name][2] = getattr(row, name)
                print("*********** results[name][2]", results[name][2])
            elif i == indices[name]+2:
                results[name][1] = getattr(row, name)
                print("*********** results[name][1]", results[name][3])
            elif i == indices[name]+3:
                results[name][2] = getattr(row, name)
                print("*********** results[name][2]", results[name][4])
    # data = {}
    # for name in results:
...
# files = glob.glob("/work/olavoryk/DT25_smhtt/smhtt_ul/output/datacards/2022_07_v6-try_no_qqh_shape_v1_new1/2018_tauid_medium/htt_mt_*/*MultiDimFit.mH125.root" )
files = glob.glob("/work/olavoryk/DT25_smhtt/smhtt_ul/*MultiDimFit.mH125.root")
...
# pt_binning = ["EMB_Pt20to25","EMB_Pt25to30","EMB_Pt30to35","EMB_Pt35to40","EMB_PtGt40"]
