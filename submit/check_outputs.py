...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=["et", "mt", "tt", "em"],
        nargs="*",
        help="Channels that have been submitted.",
    )
    parser.add_argument(
        "-t", "--tag", type=str, required=True, help="Given tag of the submission."
    )
    parser.add_argument(
        "--control", action="store_true", help="Check for control plots"
    )
...
def check_output_files(era, channel, process_string, control_arg):
    out_graph_ranges = list(
        fi.split("-")[-1].split(".root")[0]
        for fi in os.listdir(
            os.path.join(
                "output/shapes",
                "{}_unit_graphs-{}-{}-{}".format(
                    control_arg, era, channel, process_string
                ),
            )
        )
    )
    out_nums = [
        list(range(*calculate_range(out_graphs)))
        if "_" in out_graphs
        else int(out_graphs)
...
def main(args):
    proc_dict = {
        ...
        "mssm_bbhpowheg": [
            os.environ["BBH_POWHEG_SPLIT{}".format(i)] for i in range(1, 3)
        ],
        "mssm_gghpowheg": [
            os.environ["GGH_POWHEG_SPLIT{}".format(i)] for i in range(1, 4)
        ],
            ...
            for proc_str in proc_splits:
                ...
                with open(
                    os.path.join(
                        "output/submit_files",
                        "{}-{}-{}-{}-{}".format(
                            args.era, ch, proc_str, 1 if args.control else 0, args.tag
                        ),
                        "{}_unit_graphs-{}-{}-{}.pkl".format(
                            c_arg, args.era, ch, proc_str
                        ),
                    ),
                    "rb",
                ) as f:
                ...
                num_outputs, output_nums = check_output_files(
                    args.era, ch, proc_str, c_arg
                )

                print(
                    "[INFO] Checking outputs for channel {} and processes {}".format(
                        ch, proc_str
                    )
                )
                if num_graphs != num_outputs:
                    print(
                        "\033[93m[WARNING] Outputs missing for channel {}"
                        " and processes {}\033[0m".format(ch, proc_str)
                    )
                    # For deviations check which graphs are missing.
                    print(
                        "Missing outputs are {}".format(
                            sorted(set(range(num_graphs)) - output_nums)
                        )
                    )
    return
